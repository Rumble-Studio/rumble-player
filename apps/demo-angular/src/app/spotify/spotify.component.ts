...
export class SpotifyComponent implements OnInit, AfterViewInit {
  ...
  imageURL = ''
	...
	constructor(public dialog: MatDialog,
              public bridge: PlayerBridgeService) {
	  //
  }

	ngOnInit(): void {
	  this.bridge.playerHTML = new PlayerHTML()
    this.bridge.playerService = new PlayerService()
    this.bridge.playerHTML.setPlayerService(this.bridge.playerService)
  }
  ngAfterViewInit() {
	  if (this.image){
      this.bridge.playerHTML.addEventListener('newPlaylist',(payload:CustomEvent)=>{
	  ...
	  if (this.linearBar){
      this.linearBar.nativeElement.parentElement.addEventListener('click', (event)=>{
	    ...
	    this.bridge.playerHTML.addEventListener('newPosition',(payload: CustomEvent)=>{
